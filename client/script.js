async function getContacts() {
  try {
    const response = await fetch('http://localhost:3333/api/contacts/list');
    if (response.ok) {
...
async function getListMessages() {
  try {
    const response = await fetch('http://localhost:3333/api/contacts/listMessages');
    if (response.ok) {
      const data = await response.json();
...
form.addEventListener("submit", async function (event) {
  ...
  const response = await fetch("http://localhost:3333/api/contacts/add", {
    method: "POST",
    headers: {
